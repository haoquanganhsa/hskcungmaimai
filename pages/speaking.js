...
function similarity(a, b) {
  ...
  // So sánh từng ký tự (tiếng Trung mỗi chữ là 1 ký tự)
  let match = 0;
  const aChars = [...a];
  const bChars = [...b];
...
export default function Speaking() {
  ...
  const [result, setResult] = useState(null);    // { heard, pct }
  ...
  const startListening = () => {
    ...
    rec.onresult = (e) => {
      // Lấy kết quả tốt nhất trong các lựa chọn
      let best = '';
      let bestPct = 0;
    ...
    rec.onerror = (e) => {
      ...
      else if (e.error === 'not-allowed') alert('Vui lòng cho phép trình duyệt dùng microphone!');
    };
  ...
  if (!supported) return (
    <div className="max-w-lg mx-auto px-4 py-16 text-center">
      ...
      <h2 className="text-xl font-bold text-gray-700 mb-2">Trình duyệt không hỗ trợ</h2>
      <p className="text-gray-500">Vui lòng dùng <strong>Chrome</strong> hoặc <strong>Edge</strong> để luyện nói nhé!</p>
    </div>
  );
  ...
  return (
    <div className="max-w-xl mx-auto px-4 py-6">
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-500 text-sm">Nghe → Nói lại → Xem điểm độ chính xác</p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-4 mb-5 space-y-3">
        ...
        <div className="flex gap-2">
          {[{ k: 'word', label: '📝 Từ vựng' }, { k: 'sentence', label: '💬 Câu hội thoại' }].map(m => (
      ...
      {item && (
        <div className="bg-white rounded-2xl border border-gray-100 p-6 mb-4">
          ...
          <div className="text-center mb-6">
            <div className="flex justify-center gap-2 mb-3">
              <span className={`text-xs font-bold px-3 py-1 rounded-full
                ${item.level === 'HSK1' ? 'bg-green-100 text-green-700' : 'bg-purple-100 text-purple-700'}`}>
                {item.level || 'HSK'}
              </span>
            ...
            {/* Chữ Hán — ẩn khi chưa có kết quả để thử thách hơn */}
            <div className={`hanzi font-medium text-gray-800 mb-2 transition-all
          ...
          {/* Nút nghe mẫu */}
          <button onClick={() => speak(item.hanzi)}
            className="w-full py-3 mb-4 rounded-xl bg-blue-50 text-blue-600 font-semibold text-sm hover:bg-blue-100 active:bg-blue-200 transition-colors flex items-center justify-center gap-2">
            🔊 Nghe phát âm mẫu
          </button>
              ...
              <div className={`rounded-xl border p-4 fade-in ${grade.bg}`}>
                ...
                <div className="space-y-1.5 text-sm mb-4">
                  <div className="flex gap-2 items-start">
                    ...
                    <span className="hanzi font-bold text-gray-800">{item.hanzi}</span>
                  </div>
                  <div className="flex gap-2 items-start">
                    <span className="text-gray-400 w-16 flex-shrink-0">Bạn nói:</span>
                    <span className={`hanzi font-bold ${result.pct >= 60 ? 'text-green-600' : 'text-red-500'}`}>
                ...
                <div className="flex gap-2">
                  <button onClick={retry}
                    className="flex-1 py-3 rounded-xl border-2 border-gray-200 text-gray-600 font-semibold text-sm hover:bg-gray-50 active:bg-gray-100">
                    🔄 Thử lại
                  </button>
                  <button onClick={() => { speak(item.hanzi); }}
                    className="flex-1 py-3 rounded-xl border-2 border-blue-200 text-blue-600 font-semibold text-sm hover:bg-blue-50 active:bg-blue-100">
                    🔊 Nghe lại
                  </button>
                  <button onClick={next}
                    className="flex-1 py-3 rounded-xl bg-blue-600 text-white font-semibold text-sm hover:bg-blue-700 active:bg-blue-800">
                    Tiếp →
                  </button>
                </div>
              </div>
      ...
      {!listening && !result && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-sm text-amber-700 mb-4">
          ...
          <ol className="space-y-1 list-decimal list-inside">
            ...
            <li>Kết quả hiện ra ngay sau khi nói xong</li>
          </ol>
          <p className="mt-2 text-xs text-amber-600">⚠️ Dùng Chrome/Edge, cho phép dùng microphone khi được hỏi</p>
        </div>
      )}
          ...
          <div className="space-y-2">
            {history.slice(0, 8).map((h, i) => (
              <div key={i} className="flex items-center justify-between text-sm">
                ...
                <span className="hanzi text-gray-400 mx-2 flex-1 truncate">{h.heard}</span>
